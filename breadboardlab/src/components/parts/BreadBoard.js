...
export default class BreadBoard extends React.Component {
	...
	connectPart(id, partID, ref) {
		...
		if (list) {
			if (!list.find(item => item.id === partID && item.ref === ref)) {
				list.push({id: partID, ref: ref});
			}
		} else {
	...
	disconnectPart(id, partID, ref) {
		...
		if (list) {
			let item = list.find(item => item.id === partID && item.ref === ref);
    ...
    render() {
		return (
			<g ref={this.node} onDoubleClick={this.onDoubleClick} transform={`translate(6, 30) scale(0.3, 0.3)`}>
				...
				<g id="powerTop">
					<ellipse
						fill="#000000" stroke="#000000" strokeWidth="0.5" strokeOpacity="1"
						id="Power-Top-3"
						cx="12.3"
						cy="4.4"
						rx="0.5"
